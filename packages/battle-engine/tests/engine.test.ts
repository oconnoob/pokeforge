...
describe("resolveTurn", () => {
  ...
  it("executes move behavior function blobs with bounded effects", () => {
    const playerTemplate = makePokemon({
      id: "p",
      name: "Playermon",
      moves: [
        {
          ...
          behaviorFunction: `
            return {
              ...
              shieldThreshold: 30,
              healSelfPct: 0.1,
              logMessage: "Function shield online."
            };
          `
        }
      ]
    ...
    const next = resolveTurn(createBattle(playerTemplate, opponentTemplate), { moveId: "function-guard" }, sequenceRng(0, 0, 0, 0, 0));
    expect(next.log.some((event) => event.message.includes("Function shield online."))).toBe(true);
    expect(next.log.some((event) => event.message.includes("shield absorbed"))).toBe(true);
  });

  it("rejects unsafe function blobs and falls back to base attack", () => {
    const playerTemplate = makePokemon({
      id: "p",
      name: "Playermon",
      moves: [
    ...
    const next = resolveTurn(createBattle(playerTemplate, opponentTemplate), { moveId: "unsafe" }, sequenceRng(0, 0, 0, 0));
    expect(next.log.some((event) => event.message.includes("function failed safety checks"))).toBe(true);
    expect(next.opponent.currentHp).toBeLessThan(opponentTemplate.stats.hp);
  });
});
